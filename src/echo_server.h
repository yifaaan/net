...
class EchoServer {
 public:
  ...
  void HandleMessage(Connection::Ptr conn, std::string& message);

  // 数据发送完成后，在Connection类中回调
  void HandleSendComplete(Connection::Ptr conn);

  // epoll_wait超时后的回调，在EventLoop类中回调
  void HandleEpollTimeout(EventLoop* loop);

  // 给 工作线程的任务函数
  void OnMessage(std::weak_ptr<Connection> conn, const std::string& message);
