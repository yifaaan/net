...
int main(int argc, char** argv)
{
    ...
    if (argc != 3)
    {
        spdlog::error("component=echo_client event=invalid_args usage=\"./client ip port\"");
        spdlog::error("component=echo_client event=usage_example example=\"./client 192.168.101.138 5005\"");
        return -1;
    }
    ...
    {
        spdlog::error("component=echo_client event=connect_failed ip={} port={} error={}",
    ...
    for (int i = 0; i < 10; i++) // 循环3次，将与服务端进行三次通讯。
    {
        ...
        {
            spdlog::error("component=echo_client event=send_failed index={} error={}",
                          i + 1, std::strerror(errno));
            break;
        }
        spdlog::info("component=echo_client event=send_request index={} bytes={} message={}",
                     i + 1, buffer.size(), buffer);

        // 接收服务端的回应报文，如果服务端没有发送回应报文，recv()函数将阻塞等待。
        if (tcp_client.Recv(buffer, 1024) == false)
        {
            spdlog::error("component=echo_client event=recv_failed index={} error={}",
                          i + 1, std::strerror(errno));
            break;
        }
        spdlog::info("component=echo_client event=recv_reply index={} bytes={} message={}",
                     i + 1, buffer.size(), buffer);
